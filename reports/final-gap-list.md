...
|---|---|---|---|---|
| **G1** | **Opening `app\Hermes.exe` directly** bypassed `launch-hermes.cmd`: it ran without the offline `HERMES_HOME`, wrote to `%LOCALAPPDATA%\hermes`, and could attach to an online Hermes install. | **Fixed** by patch 0002 | The desktop applies the launcher's environment itself when `install-state.json` sits beside the app folder. Verified on the development laptop: a direct launch, with an online Hermes Desktop running and a user-level `HERMES_HOME` pointing at the online install, started alongside it, with its own backend (`oh…env`), home and user data, and left the online install untouched. | Re-run T8 on the VM to close. |
| **G2** | **"Repair install" / "Install Hermes locally"** started upstream's networked bootstrap installer. | **Fixed** by patch 0002 | In the offline profile the backend is pinned to the offline venv, so the resolver no longer falls through to the bootstrap. *Repair install* only restarts the backend; the first-run install choice is skipped; and if the bootstrap branch is ever reached, it refuses with "repair it offline by running scripts/install-offline.ps1 -Force". Verified on the laptop with a broken venv and with upstream's forced-bootstrap switch: no installer process started. A broken install still shows upstream's generic "couldn't start" text rather than the offline repair hint, unless the bootstrap branch is reached. | Re-run T9 on the VM to close. |
...
| **G6** | **Uninstall residue**: runs before patch 0002 left `%APPDATA%\Hermes` (Electron user data) and, after a direct launch, `%LOCALAPPDATA%\hermes`. With patch 0002 both live inside the offline home (`desktop-user-data\`). There is no uninstaller yet. | Low | Removing the install root and the home should now leave nothing but the harness's test home. | Confirm with T10c on the VM; Phase 5 uninstaller or documented steps. |
...
| **G17** | **Shared Electron user data**: the offline desktop used `%APPDATA%\Hermes`, the same folder as any online Hermes Desktop build, so the two shared local storage, caches and desktop settings, and could not run together (single-instance lock). Found while testing G1 on the development laptop. | **Fixed** by patch 0002 | The offline profile sets upstream's `HERMES_DESKTOP_USER_DATA_DIR` to `<offline home>\desktop-user-data`. Verified: the offline app ran alongside the online one with `--user-data-dir=…\desktop-user-data`. | None beyond the T8 VM re-check. |
...
Phase 4 is complete. The project as a whole is not, per `AGENTS.md`: Phase 5 (distribution) and Phase 6 (the upstream update workflow) remain. G1 and G2 are fixed by patch 0002 pending a T8/T9 re-check on the VM; G7 and G11 must still be resolved before a release.
